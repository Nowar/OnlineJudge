...
static int pickWinVoteIndex(std::vector<People> &Men) noexcept {
  std::set<People, People::WinThanObj> SortedMen {Men.begin(), Men.end() };
#ifndef ONLINE_JUDGE
  std::cerr << "Sorted Men: \n";
...
static bool checkAllTied(std::vector<People> &Men) noexcept {
  int NVote = -1;
  for (auto &M : Men) {
    if (M.isRetired())
      continue;
...
static void retireLastAndVoteAgain(std::vector<People> &Men) noexcept {
